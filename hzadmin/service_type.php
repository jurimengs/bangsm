...
if($action=="delete"){
    if(!empty($_GET['id'])){
        $id=$_GET['id'];
        #检测是否有子类
        $sub_type=$db->query_count("cn_service_type", "AND parent_id=$id");

        if($sub_type>0){
            info_sysadmin_error('该分类有子类，不能删除');
        }
        #检测是否有文章
        $product=$db->query_count("cn_service", "AND type_id=$id");
        if($product>0){
...
function display_type($db,&$typeList,$parent_id,$level){
    $result=$db->query("SELECT * FROM cn_service_type WHERE parent_id=".$parent_id." ORDER BY sort ASC,id ASC");
    while($row=$db->fetch($result)){
        $product_count=$db->query_count("cn_service","and type_id=".$row['id']);
...
$smarty->display('sysadmin/service_type.html');

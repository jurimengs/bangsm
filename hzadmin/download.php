...
if($action==""){
		...
		if(!empty($_GET['type_id'])){
			$typeid_str = '';
			getall_producttype_parent_id("cn_fuzhu_type",$typeid_str, $_GET['type_id']);
    ...
    if($tid2!='' && $tid==''){
		...
		while ($row = $db->fetch($result)){
			$result2=$db->query('SELECT name FROM cn_fuzhu_type WHERE id='.$row['type_id']);
...
if($action=="add"){
      ...
      //  $pziliao = $_POST['pimage'];
        $ziliao=$_POST['ziliao'];
        $description = str_replace("'","\'",$description);
        $contents = str_replace("'","\'",$contents);

		$time = time();
		$columnName = "title,keywords,description,contents,type_id,image,simage,time,sort,ziliao";
		$value = "'$title','$keywords','$description','$contents',$type_id,'$image','$simage',$time,$sort,'$ziliao'";
...
if($action=="edit"){
       ...
       // $pziliao = $_POST['pimage'];
	    ...
	    $mod_content = "title='$title',image='$image',
						...
						type_id=$type_id,sort=$sort,ziliao='$ziliao'
						";
		$where = 'id='.$id;
		$isbool = $db->update($table, $mod_content, $where);
		if($isbool!==false){
...
function display_type($db,&$typeList,$parent_id,$level){
	$result=$db->query("SELECT * FROM cn_fuzhu_type WHERE parent_id=".$parent_id." ORDER BY sort,id ASC");	
	while($row=$db->fetch($result)){				
...
$smarty->assign("type_id",@$_GET["type_id"]);
$smarty->display('sysadmin/download.html');

...
if($action=="delete"){
		if(!empty($_GET['id'])){
			$id=$_GET['id'];
			#检测是否有子类
			$sub_type=$db->query_count("cn_jishu_type", "AND parent_id=$id");
			
			if($sub_type>0){
				info_sysadmin_error('该分类有子类，不能删除');
			}
			#检测是否有文章
			$product=$db->query_count("cn_jishu", "AND type_id=$id");
			if($product>0){
...
if($action == "add"){
    ...
    $entitle = $_POST['entitle'];
		...
		$columnName = "name,sort,parent_id,description,entitle";
		$value = "'$name',$sort,$parent_id,'$description','$entitle'";
...
if($action=="edit"){
    ...
    $entitle = $_POST['entitle'];
		...
		$mod_content = "name='$name',sort=$sort,parent_id=$parent_id,description='$description',entitle='$entitle'";
		$where = 'id='.$id;
...
function display_type($db,&$typeList,$parent_id,$level){
	$result=$db->query("SELECT * FROM cn_jishu_type WHERE parent_id=".$parent_id." ORDER BY sort ASC,id ASC");	
	while($row=$db->fetch($result)){		
		$product_count=$db->query_count("cn_jishu","and type_id=".$row['id']);
...
$smarty->display('sysadmin/js_type.html');

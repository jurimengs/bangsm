...
if($action==""){
		...
		$nums = $db->query_count('cn_renshu',$where);
		$result1 = $db->query_page_list('cn_renshu',$page_size,$current_page,$order,$where);
		$list=array();
...
if($action == "delete"){
    ...
    if(!empty($id))
    {
            ...
            echo "<script>alert('删除成功'); location.href='yig.php';</script>";
		            if($isbool!==false){
...
if($action=="edit"){
		...
		$zong = $_POST['zong'];
				...
				$mod_content= "benqi='$benqi',
												zong='$zong'
												";
		$where = 'id='.$id;
		$isbool = $db->update($table, $mod_content, $where);
		//echo "<script>alert('修改成功'); location.href='yig.php';</script>";
		if($isbool!==false){
			success_msg('修改成功','?');
		}else{
			error_msg('修改失败','back');
...
function edit_display_type($db,&$typeList,$parent_id,$level,$type_id){
	$result=$db->query("SELECT * FROM cn_renshu WHERE parent_id=".$parent_id." ORDER BY sort,id ASC");	
...
$smarty->display('sysadmin/yig.html');

...
if($action==""){
    ...
    if(!empty($_GET['type_id'])){
        $typeid_str = '';
        getall_producttype_parent_id("cn_news_type",$typeid_str, $_GET['type_id']);
    ...
    while ($row = $db->fetch($result)){
        $result2=$db->query('SELECT name FROM cn_news_type WHERE id='.$row['type_id']);
...
function display_type($db,&$typeList,$parent_id,$level){
    $result=$db->query("SELECT * FROM cn_news_type WHERE parent_id=".$parent_id." ORDER BY sort,id ASC");
    while($row=$db->fetch($result)){
...
$smarty->assign("type_id",@$_GET["type_id"]);
$smarty->display('sysadmin/news.html');

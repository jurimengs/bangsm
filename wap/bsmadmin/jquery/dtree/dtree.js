...
| Copy (c) 2002-2003 Geir Landr?              |

|                                                   |

| This script can be used freely as long as all     |

| copy messages are intact.                    |
...
function Node(id, pid, name, url, title, target, icon, iconOpen, open,flag) {
	this.id = id;
	this.pid = pid;
	this.name = name;
	this.url = url;
...
function dTree(objName, iPath, formId) {
	...
	this.formId = (formId==null)?"rightForm":formId;
...
dTree.prototype.add = function(id, pid, name, url, title, target, icon, iconOpen, open,flag) {

	this.aNodes[this.aNodes.length] = new Node(id, pid, name, url, title, target, icon, iconOpen, open,flag);
...
dTree.prototype.node = function(node, nodeId) {
	...
	if(this.config.useCheckBox && nodeId!=0){
		...
		if(node.flag&&node.flag==1)
			str += '<input type="checkbox" name="right[]" id="c'+this.parEnum+'" onClick="caBox(\''+this.formId+'\',\'c'+this.parEnum+'\')" value="' + node.id +'" checked="checked '+'" class="cx"/>';
		else if(node.flag&&node.flag==2)
			str += '<input type="checkbox" name="right[]" id="c'+this.parEnum+'" onClick="caBox(\''+this.formId+'\',\'c'+this.parEnum+'\')" value="' + node.id +'" class="cx"/>';
		else
			str +='';
		...
		function caBox(formId, regx ) {
			if( document.getElementById( regx ).checked ){
				...
				for (var i=0; i<form.elements.length; i++) {
					...
					if (element.name == "right[]" && element.type=='checkbox'){
						// checked next node 
